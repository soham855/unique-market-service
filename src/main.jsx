...
function App() {
  ...
  const complaintModule = activeModule === 'Complaints' || activeModule === 'Raise Complaint' || activeModule === 'My Complaints'
  const adminModule = profile.role === 'admin' && !complaintModule && activeModule
  ...
  return <main className="app-shell">
    ...
    {!activeModule ? <RoleDashboard profile={profile} onSelectModule={setActiveModule} /> : complaintModule ? <section className="role-dashboard"><button className="secondary" type="button" onClick={() => setActiveModule(null)}>← Back to Dashboard</button><ComplaintModule profile={profile} /></section> : adminModule ? <AdminModule module={activeModule} onBack={() => setActiveModule(null)} /> : <section className="role-dashboard"><button className="secondary" type="button" onClick={() => setActiveModule(null)}>← Back to Dashboard</button><div className="modules"><article className="module-card"><span>●</span><h3>{activeModule}</h3><p>Module will use the live service database.</p></article></div></section>}

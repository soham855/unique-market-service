...
export default function CustomerProfile({ profile, onBack, onSaved }) {
  ...
  async function save(e) { e.preventDefault(); setSaving(true); setMessage(''); setError(''); try { const updated = await updateMyProfile(form); setMessage(updated.email !== profile?.email ? 'Profile saved. Check your new email inbox if confirmation is required.' : 'Profile updated successfully.'); onSaved?.(updated) } catch (err) { setError(err.message || 'Unable to update profile.') } finally { setSaving(false) } }
  return <section className='role-dashboard'>
    ...
    <div className='module-card' style={{maxWidth:760,margin:'24px auto',cursor:'default'}}>
      <span>●</span><h2>My Profile</h2><p>View and update your {role} details.</p>
      <form onSubmit={save} style={{display:'grid',gap:14,marginTop:18}}>
        ...
        <label>Site Address<textarea value={form.site_address} onChange={e=>change('site_address',e.target.value)} rows='3' placeholder={role==='technician' ? 'Optional service/home address' : 'Service address'} /></label>

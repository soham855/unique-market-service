...
export default function CustomerAIAssistant({profile,onBack}){
  ...
  function analyse(){
    ...
    setResult(r)
    setConversation(c=>[...c,{from:'customer',text:text.trim()},{from:'ai',text:r?'I understood this as: '+r.title+'. I will guide you through a few safe checks first.':'I could not confidently identify the issue. I can create a service complaint for a technician to check it.'}])
    setMessage('')
  }
  ...
  async function createComplaint(){
    ...
    try{
      ...
      const {data:customer,error:customerError}=await supabase.from('customers').select('id,name,mobile,company_name,address').eq('profile_id',profile.id).maybeSingle()
      ...
      const description=['AI Assistant complaint','Customer message: '+text.trim(),diagnosis?.steps?.length?'AI basic troubleshooting shown: '+diagnosis.steps.join(' | '):'Issue could not be classified confidently.'].join('\n')
    ...
    }catch(e){setMessage(e.message||'Unable to create complaint.')}
    finally{setLoading(false)}
  }

  return <section className="complaints-panel ai-assistant-panel">
    <div className="panel-heading">
      <div><span className="badge">AI SERVICE ASSISTANT</span><h2>Tell us your CCTV problem</h2><p>Type or use your phone keyboard microphone to describe the issue. The assistant will try basic troubleshooting first.</p></div>
      <button type="button" className="secondary" onClick={onBack}>← Back</button>
    </div>
    <div className="ai-chat">
      {conversation.length===0 && <div className="module-card"><div className="module-icon">🤖</div><div><h3>Example</h3><p>“Camera बंद आहे” / “My camera is offline” / “Recording दिसत नाही.”</p></div></div>}
      {conversation.map((m,i)=><div key={i} className={m.from==='customer'?'ai-message customer':'ai-message'}><strong>{m.from==='customer'?'You':'AI Assistant'}</strong><p>{m.text}</p></div>)}
      {result && <div className="ai-diagnosis"><h3>Basic troubleshooting</h3><ol>{result.steps.map((s,i)=><li key={i}>{s}</li>)}</ol><p className="muted">If these checks do not solve the problem, create a complaint below.</p><button type="button" onClick={createComplaint} disabled={loading}>{loading?'Creating complaint…':'Create Complaint Automatically'}</button></div>}
      {!result && text.trim() && <div className="ai-diagnosis"><p>I’m not confident enough to give a specific troubleshooting path.</p><button type="button" onClick={createComplaint} disabled={loading}>{loading?'Creating complaint…':'Create Complaint'}</button></div>}
    </div>

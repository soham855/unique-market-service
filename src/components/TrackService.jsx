...
export default function TrackService() {
  ...
  async function lookup(e) {
    ...
    try {
      ...
      if (!data?.length) throw new Error('No service record found. Please check the Customer ID / Complaint ID.')
      setComplaint(data[0])
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      const { data } = await supabase.rpc('public_service_status', { p_reference: complaint.ticket_no })
  ...
  return <main className="track-service-page">
    <div className="track-service-card">
      <div className="track-hero">
        ...
        <p>Enter your Customer ID or Complaint ID to see the latest service status.</p>
      </div>
      <form className="track-form" onSubmit={lookup}>
        <input value={reference} onChange={e => setReference(e.target.value)} placeholder="Customer ID or Complaint ID" autoCapitalize="characters" aria-label="Customer ID or Complaint ID" />
      ...
      {complaint && <section className="track-result" aria-live="polite">
        <div className="track-summary"><div><small>COMPLAINT ID</small><strong>{complaint.ticket_no}</strong></div><div><small>CUSTOMER ID</small><strong>{complaint.customer_code || '—'}</strong></div><div><small>LAST UPDATED</small><strong>{complaint.updated_at ? new Date(complaint.updated_at).toLocaleString('en-IN') : '—'}</strong></div></div>
        ...
        <div className="track-details"><div><small>Service</small><strong>{complaint.category || '—'}</strong></div><div><small>Complaint ID</small><strong>{complaint.ticket_no}</strong></div><div><small>Raised</small><strong>{complaint.created_at ? new Date(complaint.created_at).toLocaleString('en-IN') : '—'}</strong></div></div>
      </section>}

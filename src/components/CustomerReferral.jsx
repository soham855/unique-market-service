...
export default function CustomerReferral({profile,onBack}){
 ...
 async function submit(){
  ...
  const rule=rules[0]
  const {error}=await supabase.from('referrals').insert({referral_code_id:code.id,referrer_customer_id:customer.id,referred_name:name.trim(),referred_mobile:mobile.trim(),referred_company:company.trim()||null,reward_type:rule?.reward_type||'cash',reward_value:rule?.reward_value||500,reward_text:rule?.reward_text||'₹500 reward'})
  if(error)setMessage(error.message);else{setName('');setMobile('');setCompany('');setMessage('Referral submitted successfully.');await load()}
 }
 ...
 {loading?<p className="muted">Loading referral account…</p>:<><div className="referral-code-card"><span>Your Referral Code</span><strong>{code?.code}</strong><button onClick={()=>navigator.clipboard?.writeText(code?.code||'')}>Copy Code</button></div>
 <div className="referral-grid"><label>Customer Name<input value={name} onChange={e=>setName(e.target.value)} placeholder="Referred customer name"/></label><label>Mobile<input value={mobile} onChange={e=>setMobile(e.target.value)} placeholder="10-digit mobile"/></label><label>Company (optional)<input value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company name"/></label></div>
 <button type="button" onClick={submit}>Submit Referral</button><div className="form-section-title">Active Reward Rules</div><div className="referral-rules">{rules.map(r=><div className="module-card" key={r.id}><h3>{r.reward_text||r.name}</h3><p>{r.qualifying_condition||'Reward after successful conversion.'}</p></div>)}</div>

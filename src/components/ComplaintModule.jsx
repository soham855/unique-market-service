...
export default function ComplaintModule({ profile }) {
  ...
  async function load() {
    if (!supabase) return
    const { data, error } = await supabase.from('complaints').select('id,title,description,category,priority,status,address,created_at,customer_id,technician_id').order('created_at', { ascending:false })
  ...
  return <section className="complaints-panel">
    <div className="panel-heading"><div><span className="badge">SERVICE DESK</span><h2>Complaint Management</h2><p>Raise, assign and track service complaints.</p></div><button className="secondary" onClick={load}>Refresh</button></div>
